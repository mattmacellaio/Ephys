...
[x_binned,x_cents,x_edges,x_occ] = adaptbin(reshape(xdata,1,numel(xdata)),nBins_x);
x_binned = reshape(x_binned,nWins,size(xdata,2));
[y_binned,y_cents,edges,y_occ] = histbin(reshape(ydata,1,numel(ydata)),10);
y_binned = reshape(y_binned,nWins,size(ydata,2));

[x_binned_shuffle,x_cents_shuffle,x_edges_shuffle,x_occ_shuffle] = adaptbin(reshape(xdata_shuffle,1,numel(xdata)),nBins_x);
x_binned_shuffle = reshape(x_binned_shuffle,nWins,size(xdata,2));
[y_binned_shuffle,y_cents_shuffle,y_edges_shuffle,y_occ_shuffle] = histbin(reshape(ydata_shuffle,1,numel(ydata)),nBins_y);
y_binned_shuffle = reshape(y_binned_shuffle,nWins,size(ydata,2));

[y_binned_1shuffle,y_cents_1shuffle,y_edges_1shuffle,y_occ_1shuffle] = histbin(reshape(ydata_1shuffle,1,numel(ydata)),nBins_y);
